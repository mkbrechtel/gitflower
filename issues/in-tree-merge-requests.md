...
**The MR id is a UUIDv7** (leaning — see open questions for how it is recorded).
...
### O1 — How is the UUIDv7 id recorded?

The id must be readable from the repository itself, not only from the ref name. Candidates: a trailer on the opening commit (e.g. `MR-Id:`), generated by the author's tooling at commit time; or allocated by the hook, which then has no in-history home for it. Tied to this: is UUIDv7 confirmed as the id scheme at all, and how are ids abbreviated for humans (web URLs, CLI arguments)?

### O2 — What counts as an "MR-related commit" for push enforcement?
...
### O3 — Must the MR be up to date with the target at merge time?
...
### O4 — What is a "clean" target-update merge?
...
## Rejected: branch name or sequential number as MR id

Branch-derived ids break on branch rename and forbid repeated MRs from one branch; hook-allocated sequential numbers need atomic server-side allocation and have no in-history home. UUIDv7 is time-ordered, collision-free, and mintable client-side — at the cost of readability (see O1).
